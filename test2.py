...
# ---------------------------------------------------------
# Prediction Function with New Rules
# ---------------------------------------------------------
def get_salary(year, job, exp, size, method="Growth-Based (Recommended)"):
    ...
    # ====================================================
    # A) First, try to return ACTUAL DATA (only 2020–2022)
    # ====================================================
    if 2020 <= year <= 2022:
        exact = df[
            (df["work_year"] == year) &
        ...
        # ↓↓↓ continue to prediction stages


    # ====================================================
    # B) Prediction for 2023–2030 OR if no actual exists
    # ====================================================
    ...
    # -----------------------------------------
    # RULE 3 → Profile has 0 years of data
    # -----------------------------------------
    if num_years == 0:

        # 3a: Try Yearly Market Average (same year)
    ...
    if num_years == 1:
        ...
        predicted = base_salary * ((1 + avg_growth) ** years_ahead)

        return float(predicted), "Predicted (Similar Profile Pattern)"


    # -----------------------------------------
    # RULE 1 → Profile has ≥ 2 years of data
    # -----------------------------------------
    if num_years >= 2:
        ...
        years_ahead = year - base_year
        predicted = base_salary * ((1 + growth_rate) ** years_ahead)

        return float(predicted), f"Predicted (Own Pattern: {growth_source})"


    # ====================================================
    # FINAL BACKUP (should never run, but safe)
    # ====================================================
...
with tab1:
    ...
    # Show growth / data availability info (based on new rules)
    profile_history = df[
